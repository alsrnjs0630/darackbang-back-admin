...
public class Payment extends AbstractAuditingEntity implements Serializable {
    ...
    //주문자 우편번호
    @Column(name = "buyer_postcode", nullable = false, length = 7)
    private String buyerPostcode;

    //주문자 연락처
    @Column(name = "buyer_tel", nullable = false, length = 13)
    ...
    //할부 개월
    //기본값이 00
    @Column(name = "card_quota", nullable = false, length = 2)
    ...
    //회원번호
    @Column(name = "customer_uid", nullable = false, length = 50)
    ...
    //결제금액
    @Column(name = "paid_amount", nullable = false, length = 7)
    private Integer paidAmount;

    //결제승인시각, 결제일
    // Unix timestamp 사용시 string type, Mapper로 LocalDateTime으로 변환
    @Column(name = "paid_at", nullable = false, length = 10)
    private String paidAt;

    //결제수단 구분코드
    @Column(name = "paid_method", nullable = false, length = 15)
    private String payMethod;

    //PG사 구분코드
    @Column(name = "pg_provider", nullable = false, length = 15)
    ...
    @Column(name = "pg_type", nullable = false, length = 15)
    ...
    // 결제 실패 원인
    @Column(name = "fail_reason", nullable = false, length = 1000)
